            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="box box-warning">
                        <div class="box-body">
                            <?php echo form_open_multipart(base_url('cart/set_nota.php'), 'id="form-nota" onsubmit="return form_validasi()"') ?>
                                    ...
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div id="bag-pelanggan" class="form-group <?php echo (!empty($hasError['pelanggan']) ? 'has-error' : '') ?>">
                                                ...
                                                <div class="input-group date">
                                                    <input type="hidden" id="idpel" name="pelanggan">
                                                    <?php echo form_input(array('id' => 'pelanggan', 'name' => 'pelanggan2', 'class' => 'form-control pull-right')) ?>
                                                    <!--
                                                    <select id="pelanggan" name="pelanggan" class="form-control select2">
                                                        <option value="">- [Pilih] -</option>
                                                        <?php
//                                                        $plgn = $this->db->where('status_hps', '0')->get('tbl_m_pelanggan')->result();
//                                                        if (!empty($plgn)) {
//                                                            foreach ($plgn as $plgn) {
//                                                                $sql_tipe = $this->db->where('id', $plgn->id_grup)->get('tbl_m_pelanggan_grup')->row();
                                                                ?>
                                                                <option value="<?php // echo $plgn->id ?>" <?php // echo ($plgn->id == '1' ? 'selected' : '') ?>><?php // echo '[' . $plgn->kode . '] ' . $plgn->nama.' '.$plgn->no_hp. ' - ' . $sql_tipe->grup ?></option>
                                                                <?php
//                                                            }
//                                                        }
                                                        ?>
...
<!--                                            <div id="msg-success" class="alert alert-success alert-dismissible">
                            ...
                            <div class="row">
                                <div id="keranjang" class="col-lg-4">
                                    ...
                                    <button type="submit" id="btn-submit" class="btn btn-primary btn-flat" onclick="return form_validasi();">Set Order</button>
                                </div>
                            </div>
                            <?php echo form_close() ?>
                        </div>
                    ...
                    <div class="col-lg-8">
                        <div class="box box-warning" id="data_pelanggan">
                            ...
                            <div class="box-footer">
                                <button class="btn btn-primary btn-flat pull-right" onclick="window.location.href = '<?php echo base_url('cart/cart-step-1.php') ?>'">Lanjut &raquo;</button>
                            </div>
                        </div>
                    </div>
                </div>
...
// Tanggal bayar
  ...
  $('#pelanggan').autocomplete({
      source: function (request, response) {
          $.ajax({
              url: "<?php echo site_url('page=home&act=json_pelanggan') ?>",
              dataType: "json",
              data: {
                  term: request.term
              },
      ...
      select: function (event, ui) {
          ...
          $itemrow.find('#idpel').val(ui.item.id);
  ...
  //Format the list menu output of the autocomplete
      ...
      return $("<li></li>")
              ...
              .append("<a>[" + item.kode + "] " + item.nama + " / " + item.no_hp + " / " + item.alamat + " / " + item.grup + "</a>")
              .appendTo(ul);
  };
