    ...
    public function cart_simpan() {
        if (akses::aksesLogin() == TRUE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data = array(
                    'id'      => rand(1,32).general::dekrip($id_kat2),
                    'qty'     => (!empty($jml) ? (int)$jml : 1),
...
//            print_r($data_trans);
            ...
            foreach ($this->cart->contents() as $item) {
                ...
                $jml_promo  = $jml_promo + $item['options']['promo'];
            ...
            $data_trans_upd = array(
                'jml_diskon'  => round($jml_promo, -2),
//                'jml_gtotal'  => round($subtot_disk, -2),
            );
//            crud::update('tbl_trans_jual','id',$sql->id, $data_trans_upd);
    ...
    public function trans_order_list() {
        if (akses::aksesLogin() == TRUE) {
            ...
            $tgl= explode('/', $tg);
            /* -- End Blok Filter -- */
//            echo ($grup->name == 'superadmin' || $grup->name == 'owner' || $grup->name == 'admin' ? 'root' : $id_user);
...
//            $where = "(tbl_trans_jual.id_app LIKE '".(!empty($lk) ? $lk : "%%")."')";    
            $where = (!empty($lk) ? "(tbl_trans_jual.id_app = '".$lk."')" : "(tbl_trans_jual.id_app LIKE '%%')");    
            if(!empty($lk)){
                $jml_hal = $this->db->select('tbl_trans_jual.id')
                           ->join('tbl_m_pelanggan', 'tbl_m_pelanggan.id=tbl_trans_jual.id_pelanggan', 'left')
                           ->like('id_user', ($grup->name == 'superadmin' || $grup->name == 'owner' || $grup->name == 'admin' ? '' : $id_user))
                           ->where($where)
...
//                           ->like('tbl_trans_jual.status_bayar', $sb)
            ...
            /* -- Blok Pagination -- */
            $config['base_url']              = base_url('cart/trans_order_list.php?filter_nota='.$nt.'&filter_lokasi='.$lk);
            ...
            // OR tbl_trans_jual.status_bayar LIKE '".(!empty($sb) ? $sb : "%%")."'
            if(!empty($hal)){
                if(!empty($lk)){
                    $data['penj'] = $this->db->select('tbl_trans_jual.id, tbl_trans_jual.id_app, tbl_trans_jual.no_nota, DATE(tbl_trans_jual.tgl_masuk) as tgl_masuk, DATE(tbl_trans_jual.tgl_bayar) as tgl_bayar, DATE(tbl_trans_jual.tgl_ambil) as tgl_ambil, tbl_trans_jual.jml_total, tbl_trans_jual.jml_gtotal, tbl_trans_jual.pengambilan, tbl_trans_jual.id_user, tbl_trans_jual.status_nota, tbl_trans_jual.status_bayar, tbl_trans_jual.cetak, tbl_m_pelanggan.nama, tbl_m_pelanggan.no_hp')
                           ->join('tbl_m_pelanggan', 'tbl_m_pelanggan.id=tbl_trans_jual.id_pelanggan', 'left')
                           ->limit($config['per_page'],$hal)
                           ->where($where)
                           ->like('id_user', ($grup->name == 'superadmin' || $grup->name == 'owner' || $grup->name == 'admin' ? '' : $id_user))
                           ->like('tbl_trans_jual.no_nota', $nt)
//                           ->or_like('tbl_trans_jual.id_app', $lk)
//                           ->or_like('tbl_m_pelanggan.nama', $nt)
//                           ->or_like('tbl_m_pelanggan.no_hp', $nt)
...
//                           ->or_like('tbl_trans_jual.id_app', $lk, 'match')
                           ->order_by('tbl_trans_jual.no_nota','desc')
                           ->get('tbl_trans_jual')->result();                    
                }else{
                   $data['penj'] = $this->db->select('tbl_trans_jual.id, tbl_trans_jual.id_app, tbl_trans_jual.no_nota, DATE(tbl_trans_jual.tgl_masuk) as tgl_masuk, DATE(tbl_trans_jual.tgl_bayar) as tgl_bayar, DATE(tbl_trans_jual.tgl_ambil) as tgl_ambil, tbl_trans_jual.jml_total, tbl_trans_jual.jml_gtotal, tbl_trans_jual.pengambilan, tbl_trans_jual.id_user, tbl_trans_jual.status_nota, tbl_trans_jual.status_bayar, tbl_trans_jual.cetak, tbl_m_pelanggan.nama, tbl_m_pelanggan.no_hp')
...
//                           ->like('status_nota', $sn)
    ...
    public function trans_bayar_list() {
        if (akses::aksesLogin() == TRUE) {
            ...
            /* -- jml halaman pada list -- */
            if(akses::hakSA() == TRUE OR akses::hakAdmin() == TRUE){
                $jml_hal = (!empty($jml) ? $jml  : $this->db->get('tbl_trans_jual')->num_rows());
            }else{
                /* -- Hitung jumlah halaman -- */
                $jml_hal = (!empty($jml) ? $jml  : $this->db->get('tbl_trans_jual')->num_rows());
            }
...
//                           ->where('tbl_trans_jual.id_user', $id_user)
    ...
    public function trans_detail_edit_ket() {
        if (akses::aksesLogin() == TRUE) {
            $setting            = $this->db->get('tbl_pengaturan')->row();
            $id                 = $this->input->get('id');
            ...
            $this->load->view('admin-lte-2/1_atas',$data);
            $this->load->view('admin-lte-2/2_header',$data);
            $this->load->view('admin-lte-2/includes/trans/trans_detail_edit_ket',$data);
            $this->load->view('admin-lte-2/5_footer',$data);
            $this->load->view('admin-lte-2/6_bawah',$data);
        } else {
            $errors = $this->ion_auth->messages();
    ...
    public function trans_detail_edit_ket_upd() {
        if (akses::aksesLogin() == TRUE) {
            $id     = $this->input->post('id');
            ...
            $this->form_validation->set_rules('itemid', 'ID Item', 'required');

            if ($this->form_validation->run() == FALSE) {
                $msg_error = array(
                    'rowid' => form_error('itemid'),
                );
                ...
                crud::update('tbl_trans_jual_det','id', $iditem, $data);
                
                redirect(base_url('cart/trans_edit_ket.php?id='.$id.'&itemid='.general::enkrip($iditem)));
            }
        } else {
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login', '<div class="alert alert-danger">Authentifikasi gagal, silahkan login ulang!!</div>');
...
//            $objPHPExcel->getActiveSheet()->getRowDimension(1)->setRowHeight(-1);
            ...
            $objPHPExcel->getActiveSheet()->getStyle('C5:F5')->getFont()->setSize('10')->setName('Tahoma');
            $objPHPExcel->getActiveSheet()->getStyle('A4:B4')->getFont()->setSize('10')->setName('Tahoma')->setBold(TRUE);
...
//                    ->setCellValue('I3', '');
            // Kontak Nota
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A4', 'NOTA PEMESANAN')->mergeCells('A4:B4')
                    ->setCellValue('C4', $sql->row()->nama.' ('.$sql_grup->row()->grup.') '.$sql->row()->no_hp)->mergeCells('C4:F4');
            

            // NOTA PEMESANAN
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A5', '')->mergeCells('A5:B5')
                    ->setCellValue('C5', $sql->row()->alamat)->mergeCells('C5:F5');
...
//            $objPHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(13);
            ...
            foreach ($sql_det->result() as $sql_det){
                $sql_kat2= $this->db->where('id', $sql_det->id_kategori2)->get('tbl_m_kategori2')->row();
		...
		$pcs     = $pcs + (($sql_det->jml * $sql_det->pcs) * $sql_kat2->jml);
...
//                    ->setCellValue('G' . $sell2, '')
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A' . $sellbwh1, ($sql->row()->id_promo != 0 ? $sql_promo->keterangan.' '.number_format($sql_promo->persen,'0','.',',').'%' : ''))->mergeCells('A' . $sellbwh1.':C' . $sellbwh1)
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('F' . $sellbwh2, ($sql->row()->jml_gtotal == 0 ? number_format($sql->row()->jml_total - $sql->row()->jml_diskon,'0','.',',') : number_format($sql->row()->jml_gtotal,'0','.',',')));
...
//            $objPHPExcel->getActiveSheet()->getStyle('A' . $sellbwh3.':C'.$sellbwh3)->get()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A' . $sellbwh3, ($sql->row()->cetak == '1' ? 'RE-PRINT' : ''))->mergeCells('A' . $sellbwh3.':B' . $sellbwh3)
    ...
    public function set_lokasi_rak() {
        if (akses::aksesLogin() == TRUE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                crud::update('tbl_trans_jual', 'id', $id, array('status_trx'=>'1'));
...
//                    print_r($trans);
                ...
                if(!empty($pengambil)){
                   $data_ambil = array(
                       ...
                       'id_pengambilan' => $this->ion_auth->user()->row()->id,
        ...
        if (akses::aksesLogin() == TRUE) {
            ...
            $tgl      = explode('/', $tanggal); // mm/dd/yy 012
                   ...
                   $sql = $this->db->select('tbl_trans_jual.id, tbl_trans_jual.id_app, tbl_trans_jual.no_nota, DATE(tbl_trans_jual.tgl_masuk) as tgl_masuk, DATE(tbl_trans_jual.tgl_bayar) as tgl_bayar, tbl_trans_jual.jml_total, tbl_trans_jual.jml_gtotal, tbl_trans_jual.pengambilan, tbl_trans_jual.id_user, tbl_trans_jual.status_nota, tbl_trans_jual.status_bayar, tbl_trans_jual.cetak, tbl_m_pelanggan.nama, tbl_m_pelanggan.no_hp')
                           ->join('tbl_m_pelanggan', 'tbl_m_pelanggan.id=tbl_trans_jual.id_pelanggan', 'left')
//                           ->like('id_user', ($grup->name == 'superadmin' || $grup->name == 'owner' || $grup->name == 'admin' ? '' : $id_user))
                           ->like('tbl_trans_jual.id_app', $lokasi)
