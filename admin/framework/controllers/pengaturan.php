...
//                        $sql = $this->db->select('MAX(id) as id')->get('tbl_trans_jual')->row();
                        ...
                        foreach ($json['tbl_trans_jual'] as $kat) {
                            ...
                            if($sql_cek->num_rows() == 0){
                                ...
                                // Update lokasi
                                $this->db->where('id_penjualan', $nota_id)->delete('tbl_trans_jual_lokasi');
                                foreach ($kat['tbl_trans_jual_lokasi'] as $trans_lokasi){
                                    $tbl_trans_jual_lokasi = array(
                                        'id_app'        => $trans_lokasi['id_app'],
...
//                            }
                        ...
                        foreach ($json['tbl_m_lokasi'] as $lok) {
                            $sql_cek = $this->db->where('id',$lok['id'])->get('tbl_m_lokasi');
                            
                            $tbl_m_lokasi = array(
                                'id'           => $lok['id'],
                                'id_app'       => $lok['id_app'],
                                'tgl_simpan'   => $lok['tgl_simpan'],
                                'kode'         => $lok['kode'],
                            ...
                            if($sql_cek->num_rows() == 0){
                                crud::simpan('tbl_m_lokasi', $tbl_m_lokasi);
                            }else{
                                crud::update('tbl_m_lokasi', 'id', $lok['id'], $tbl_m_lokasi);
                            }
                        }
