...
class member extends CI_Controller {
    ...
    public function member_list() {
        if (akses::aksesLogin() == TRUE) {
            ...
            $config['base_url']               = base_url('member.php?'.(isset($_GET['query']) ? '&query='.$_GET['query'].'&jml='.$_GET['jml'] : ''));
            ...
            if(!empty($hal)){
                ...
                if (!empty($query)) {
                    $data['pelanggan'] = $this->db->where('status_hps', '0')->like('nama', $query)->limit($config['per_page'])->get('tbl_m_pelanggan')->result();
                } else {
    ...
    public function member_simpan() {
        if (akses::aksesLogin() == TRUE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                crud::update('tbl_m_pelanggan_deposit','id_pelanggan', general::dekrip($id), array('status_plgn'=>'1'));
                crud::update('tbl_m_pelanggan_deposit_hist','id_pelanggan', general::dekrip($id), array('status_plgn'=>'1'));
                redirect(base_url('member/member_ubah.php?id='.$id));
            }
        } else {
            $errors = $this->ion_auth->messages();
...
//            }
    ...
    public function set_cari_plgn() {
        if (akses::aksesLogin() == TRUE) {
            $query  = $this->input->post('pencarian');
            
            echo $sql = $this->db->where('status_hps', '0')
                                 ->like('nama', $query)
//                            ->or_like('nama', $query)
//                            ->or_like('nik', $query)
//                            ->or_like('no_hp', $query)
//                            ->or_like('alamat', $query)
                                ->get('tbl_m_pelanggan')->num_rows();
                        
            redirect(base_url('member.php'.($sql > 0 ? '?query='.$query.'&jml='.$sql : '')));
        } else {
            $errors = $this->ion_auth->messages();
            $this->session->set_flashdata('login', '<div class="alert alert-danger">Authentifikasi gagal, silahkan login ulang!!</div>');
            redirect();
